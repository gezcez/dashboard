...
export async function makeGezcezRequest(
	...
	if (!url.startsWith(API_URL)) throw new Error(`Cant make api requests outside the ${API_URL}!`)
	const [data, request] = await fetchJSON(input, {
		method: init?.body ? "POST" : "GET",
		...init,
		headers: {
			...init?.headers,
			Authorization: `Bearer ${is_use_refresh_token ? state.refresh_token : state.access_token}`,
